<br>
<form action="/report_dir" method="post">
	...
	<br>
	<div class="form-group row">
		<div class="col"><b>Выберите врача:</b></div>
		<div class="col">
	<?php
	//    debug($DoctorsListId);
		$SelectDoctor = array2select($DoctorsListId, 'doctor_id', 'doctor_f', 'doctor_id', ' class="form-control form-control-lg"');
...
if ( count($_POST) > 0 )
{
	$DoctorData = $DoctorsListId['id' . $_POST['doctor_id']];
	if ( count($DoctorData) > 0 )
	{

//		debug($DoctorData);
//		debug($CaopPersonal['id'.$DoctorData['doctor_nurse']]);

		$unix_from = strtotime($_POST['report_dir_date_from']);
		$unix_to = strtotime($_POST['report_dir_date_to']);

		// НАХОДИМ ОБСЛЕДОВАНИЯ
		$queryResearch = "SELECT * FROM {$CAOP_RESEARCH} WHERE research_unix>='{$unix_from}' AND research_unix<='{$unix_to}' AND research_doctor_id='{$DoctorData['doctor_id']}'";
// 		debug($queryResearch);
		$resultResearch = mqc($queryResearch);
		$amountResearch = mnr($resultResearch);
// 		debug($amountResearch);
...
//		debug($ResearchesData);
		...
		// НАХОДИМ ЦИТОЛОГИЮ
		$queryCitology = "SELECT * FROM {$CAOP_CITOLOGY} WHERE citology_dir_date_unix>='{$unix_from}' AND citology_dir_date_unix<='{$unix_to}' AND citology_dir_doctor_id='{$DoctorData['doctor_id']}'";
...
//		debug($CitologysData);
		...
		<div class="print-selected" id="print-selected">
			...
			<div class="row">
				<div class="col">
					<b>Врач:</b> <?=docNameShort($DoctorData, "famimot") ?><br>
					<b>Медсестра:</b> <?=docNameShort($CaopPersonalId['id'.$DoctorData['doctor_nurse']], "famimot") ?><br>
					<b>Кабинет:</b> <?=$DoctorData['doctor_cabinet'] ?><br>
				</div>
			</div>
			<hr>
	...
	{
		bt_notice('Неверно выбран врач',BT_THEME_DANGER);
	}
	// Вылавливаем направления на обследования
