...
if ( ifound($RequestData[0], "patient") )
{
	...
	{
		$is_query = false;
		$queryCitology = "SELECT * FROM $CAOP_CITOLOGY cc LEFT JOIN $CAOP_PATIENTS cp ON cc.citology_patid=cp.patid_id WHERE citology_patid='{$user_id}' ORDER BY cc.citology_result_text ASC, cc.citology_id DESC";
	}
}
...
//	debug($ShowMePeriodCitology);
		if ( $ShowMePeriodCitology != "all" )
		{
			...
			if ( $PeriodData !== false )
			{
				...
				$queryCitology = "SELECT * FROM $CAOP_CITOLOGY cc LEFT JOIN $CAOP_PATIENTS cp ON cc.citology_patid=cp.patid_id WHERE cc.citology_dir_date_unix>='{$lastPeriod}' ".$research_query_archive_addon." ORDER BY cc.citology_result_text ASC, cc.citology_id DESC";
			}

//		debug($PeriodData);
		} else
		{
			$queryCitology = "SELECT * FROM $CAOP_CITOLOGY cc LEFT JOIN $CAOP_PATIENTS cp ON cc.citology_patid=cp.patid_id WHERE 1 ".$research_query_archive_addon." ORDER BY cc.citology_result_text ASC, cc.citology_id DESC";
		}
	} else
	{
		$queryCitology = "SELECT * FROM $CAOP_CITOLOGY cc LEFT JOIN $CAOP_PATIENTS cp ON cc.citology_patid=cp.patid_id WHERE 1 ".$research_query_archive_addon." ORDER BY cc.citology_result_text ASC, cc.citology_id DESC";
