<br>
<form action="/report_patients" method="post">
...
if ( count($_POST) > 0 ) {
	...
	$queryVisits = "SELECT * FROM {$CAOP_JOURNAL} WHERE journal_day IN (SELECT day_id FROM {$CAOP_DAYS} WHERE day_unix>='{$unix_from_accu}' AND day_unix<='{$unix_to_accu}' ORDER BY day_id DESC) GROUP BY journal_patid ORDER BY journal_id DESC";
//	debug($queryVisits);
	...
	<div class="print-selected" id="print-selected">
		...
		<div class="row">
			<div class="col">
				<b>ВСЕГО ПАЦИЕНТОВ:</b> <?=$amountVisits?><br><br>
				<b>Из них принято врачами:</b><br>
				<?php
