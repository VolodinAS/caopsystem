...
if ( $PrintParams[0] > 0 )
{
	$BlankPrint = getarr(CAOP_DAILY, "daily_id='{$PrintParams[0]}'");
	if ( count($BlankPrint) > 0 )
	{
		$BlankPrint = $BlankPrint[0];
		$PatientData = getPatientDataById($BlankPrint['daily_patid']);
		if ($PatientData['result'] == true && $PatientData['error'] == false)
		{
			$PatientData = $PatientData['data']['personal'];
			
			$DoctorData = $DoctorsListId['id' . $BlankPrint['daily_doctor_id']];
			
			require_once ( "engine/html/title_begin_print.php" );
			require_once ( "engine/html/dailyHtml.php" );
