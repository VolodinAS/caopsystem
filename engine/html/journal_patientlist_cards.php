...
$queryPatientsToday = "SELECT IF( cj.journal_recom = '', 0, 1 ) AS status, cj . *, cp . * FROM {$CAOP_JOURNAL} cj INNER JOIN {$CAOP_PATIENTS} cp ON cj.journal_patid=cp.patid_id WHERE cj.journal_day='{$Today_Array['day_id']}' AND cj.journal_doctor='{$CHOSEN_DOCTOR_ID}' ORDER BY status, cj.journal_time, cp.patid_name ASC";
...
$check_fields = [
	'journal_time' => 'Не указано время',
	'journal_ds' => 'Не указан диагноз по МКБ',
	'journal_ds_text' => 'Не указан текст диагноза',
	'journal_ds_recom' => 'Не указаны рекомендации',
	'journal_recom' => 'Не указан исход случая',
	'journal_cardplace' => 'Не указано место карты',
	'journal_infirst' => 'Не указан случай',
	'journal_dirstac' => 'Не указано направление в учреждение',
	'journal_need_move' => 'Есть необходимость переноса',
	'journal_dirstac_desc' => 'Не указано учреждение',
	'journal_disp_isDisp' => 'Не указана диспансеризация',
	'journal_disp_lpu' => 'Не указано ЛПУ диспансера',
	'journal_disp_mkb' => 'Не указано МКБ диспансера'
];

$PatientCheck = patientCheck($PatientsToday);
//debug($PatientCheck);
if (count($PatientCheck['journal_errors']) > 0)
{
	bt_divider();
	spoiler_begin('ЕСТЬ ОШИБКИ В ДНЕВНИКАХ: <b>' . count($PatientCheck['journal_errors']) . '</b>', 'journal_error', 'collapse', 'bg-' . BT_THEME_WARNING);
	foreach ($PatientCheck['journal_errors'] as $pat)
	{
		?>
        <div>
            <b><?= $check_fields[$pat['ERROR_FIELD']]; ?>:</b>
            <a href="#fortarget<?= $pat['journal_id']; ?>"><?= shorty($pat['patid_name'], 'famimot'); ?>
                , <?= $pat['patid_birth']; ?> г.р.</a>
        </div>
		<?php
	}
	spoiler_end();
...
if (count($PatientCheck['moves']) > 0)
{
	spoiler_begin('У Вас в дневниках есть переносы: <b>' . count($PatientCheck['moves']) . '</b>', 'moves');
	
	?>
    <button class="btn btn-sm btn-primary btn-moveAllPatients"
            data-day="<?= $Today_Array['day_id']; ?>"
            data-doctor="<?= $CHOSEN_DOCTOR_ID; ?>">Перенести всех
    </button>
	<?php
	
	foreach ($PatientCheck['moves'] as $pat)
	{
		?>
        <div>
            <b><?= $check_fields[$pat['ERROR_FIELD']]; ?>:</b> <?= shorty($pat['patid_name'], 'famimot'); ?>
            , <?= $pat['patid_birth']; ?> г.р.
        </div>
		<?php
	}
...
if (count($PatientsToday) > 0)
{
    ...
    <table class="table tablesorter table-sm"
           id="patient_journal">
        <thead>
        <tr class="header-sticky bg-white">
            <th scope="col"
                width="5%"
                align="center"
                class="sorter-false"
            >
                ...
                <input class="form-check-input " type="checkbox" name="check_all" id="check_all" value="1" >
                <label class="form-check-label box-label" for="check_all"><span></span></label>
            </th>
...
//			$PatientPersonalData = getarr(CAOP_PATIENTS, "patid_id='{$Patient['journal_patid']}'");
			$PatientPersonalData = extractValueByKey($Patient, 'patid_');
...
//            debug($PatientPersonalData);
			...
			if (count($PatientPersonalData) > 0)
			{
				...
				$dead = patient_dead($PatientPersonalData);
				$mark = patient_mark($PatientPersonalData);
...
//                debug($CaseStatusesListId);
			...
			if ( $Patient['journal_visit_type'] == 2 )
            {
	            $patient_status = wrapper('ПРОВЕДЕНА ТЕЛЕМЕДИЦИНА');
            }
...
//			debug($PatStat);
			...
			$DispPatient = '';
//			$DispancerSPO = getarr(CAOP_SPO, "spo_patient_id='{$PatientPersonalData[$PK[CAOP_PATIENTS]]}' AND spo_is_dispancer='1'");
			$DispancerSPO = getarr(CAOP_SPO, $PK[CAOP_SPO] . "='{$Patient['journal_spo_id']}' AND spo_is_dispancer='1'");
			if (count($DispancerSPO) > 0)
			{
				$DispancerSPO = $DispancerSPO[0];
				$LPU = $DispLPUId['id' . $DispancerSPO['spo_lpu_id']]['lpu_showname'];
				$DispPatient = '<span ' . super_bootstrap_tooltip($DispancerSPO['spo_mkb_finished'] . ' от '.date(DMY, $DispancerSPO['spo_unix_accounting_set']).'; ' . $LPU) . '>' . BT_ICON_DISPANSER . '</span>';
			}
...
//			}
          	...
          	$visit_type = ($Patient['journal_visit_type'] == 2) ? ' bg-info' : '';
          	
			
			?>
            <tr class="<?= $light_bg; ?> <?=$visit_type;?>"
                id="fortarget<?= $Patient['journal_id']; ?>">
                <td data-title="#"
                    align="center">
                    <!--                    <a name="patient--><?//= $Patient['journal_id'];
					                                           ?><!--"></a>-->
                ...
                <td data-title="Открыть"
                    ...
                    <button <?= super_bootstrap_tooltip('Редактировать дневник'); ?> type="button"
                                                                                     class="btn btn-primary btn-sm openCard"
					...
					{
						?>
                        <!--                        <button --><?//= super_bootstrap_tooltip('ОТКРЫТЬ СПО');
						?><!-- type="button" class="btn btn-danger btn-sm openCardSPO" data-patient="--><?//= $Patient['journal_id'];
						?><!--">-->
                ...
                <td data-title="Карта"
                    align="center">
                    <span class="small"><?= $PatientPersonalData['patid_ident']; ?></span>
                </td>
                <td data-title="CR">
					<?= $cancer_image; ?>
                </td>
                ...
                <td data-title="Время">
                    <span class="small"><?= $Patient['journal_time']; ?></span>
                </td>
                ...
                <td data-title="Ф.И.О.">
					<?= $dead; ?><?=$mark;?><?= editPersonalDataLink(shorty($PatientPersonalData['patid_name']), $PatientPersonalData['patid_id'], super_bootstrap_tooltip(mb_ucwords($PatientPersonalData['patid_name']))); ?>
                ...
                <td data-title="Диагноз">
					...
					if (strlen($Patient['journal_ds_text']) > 0)
					{
						...
						?>
                        <span class="size-12pt badge badge-<?= $badge; ?>"><?= $Patient['journal_ds']; ?></span> <?= $Patient['journal_ds_text']; ?>
						<?php
					}
					?>
					<?php
                ...
                <td align="center">
					<?php
					$btn = (strlen($Patient['journal_need_move']) < 10) ? html_img_gen("/engine/images/icons/journal_not_need_move.png", 'height="24"', 1) : html_img_gen("/engine/images/icons/journal_need_move.png", 'height="24"', 1);
                    ...
                    <!--                          data-returnfunc="journal_current_move_button">--><?//= $btn;
					?><!--</span>-->
                    <span class="cursor-pointer"
                          id="fastMoveButtonIcon<?= $Patient['journal_id']; ?>"
                          onclick="javascript:journal_fastMove(<?= $Patient['journal_id']; ?>)">
                        <?= $btn; ?>
                    </span>
                ...
                <td align="center">
                    <div class="dropdown div-jinline dropleft"
                        ...
                        <div class="dropdown-menu"
                            ...
                            <a class="dropdown-item"
                               href="javascript:journalResearch2(<?= $Patient['journal_id']; ?>, true)">Записать
                                                                                                  на обследование</a>
                            <a class="dropdown-item"
                               href="javascript:journalCitology2(<?= $Patient['journal_id']; ?>, true)">Записать
                                                                                                  на цитологию</a>
                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item"
                            ...
                            <a class="dropdown-item"
                               target="_blank"
                               href="/noticeF1A/<?= $PatientPersonalData['patid_id']; ?>/add/<?= $Patient['journal_id']; ?>">Установить
                                                                                                                             1А
                                                                                                                             клиническую
                                                                                                                             группу</a>
                            ...
                            <a class="dropdown-item"
                               target="_blank"
                               href="/dailys/<?= $PatientPersonalData['patid_id']; ?>"><b>Дневник</b>, если ЕМИАС не
                                                                                                     работает</a>
                        </div>
                    </div>
                    <div class="dropdown div-jinline dropleft"
                        ...
                        <div class="dropdown-menu"
                            ...
                            <a class="dropdown-item"
                               target="_blank"
                               href="/msktmdc/<?= $PatientPersonalData['patid_id']; ?>/add/<?= $Patient['journal_id']; ?>">
                                Направление на МСКТ в МДЦ-Медицина
                            </a>
                            <a class="dropdown-item"
                               target="_blank"
                               href="/anylpu/<?= $PatientPersonalData['patid_id']; ?>/add/<?= $Patient['journal_id']; ?>">
                                Направление другое ЛУ России
                            </a>
                            <a class="dropdown-item"
                               target="_blank"
                               href="/morphology/<?= $PatientPersonalData['patid_id']; ?>/add/<?= $Patient['journal_id']; ?>">
                            ...
                            <a class="dropdown-item"
                               target="_blank"
                               href="/blankDeclined/<?= $PatientPersonalData['patid_id']; ?>/add/<?= $Patient['journal_id']; ?>">
